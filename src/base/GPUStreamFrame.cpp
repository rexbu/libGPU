...
#pragma --mark "输入格式"
void GPUStreamFrame::setInputFormat(gpu_pixel_format_t format, GPUFilter* i){
    // 重新设置输入格式
    if (m_input!=NULL)
    {
...
void GPUStreamFrame::setExtraFilter(const char* file, const char* image){
    ...
    GPUFilter* extra_filter = NULL;
    for (int i=0; i<g_file_filter_num; i++) {
        if (strcmp(name, g_file_filter_map[2*i])==0) {
            extra_filter = new GPUFileFilter(g_file_filter_map[i*2+1]);
            break;
        }
    }
